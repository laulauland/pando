# Optional runtime packaging

The BoxLite integration is experimental and is not part of Pando's ordinary GitHub release, Homebrew bottle, source formula, install script, or mise installation surface. Those channels continue to build Cargo's empty default feature set. Runtime-enabled CI artifacts are named `pando-runtime-experimental-<target>` so they cannot be mistaken for the supported small binary.

The experimental workflow uses Rust 1.97.0 and full-SHA-pinned actions. It builds with `--locked --profile runtime-experimental --features microvm-boxlite`, smoke-tests the default and runtime-enabled binaries separately, parses the one exact macOS BoxLite shim's entitlement plist and requires `com.apple.security.hypervisor = true`, emits a checksum beside each CI artifact, and never attaches those artifacts to a GitHub release. Each manually dispatched self-hosted lane downloads that exact artifact, verifies its checksum, unpacks it, and passes its binary explicitly to the full lifecycle/crash harness. The custom unpublished profile retains debug assertions solely so the packaged qualification binary includes Pando's lifecycle crash-injection points.
...
Self-hosted labels are routing metadata, not a trust boundary. The KVM root, KVM rootless, and HVF workers must be single-job ephemeral machines (or be fully reimaged after every job), contain no long-lived repository or cloud credentials, and accept jobs only from this repository/environment. Approval must be denied if the dispatch is not the expected protected `main` revision. `scripts/check-runtime-workflow-trust` runs a locked Rust checker that parses the workflow as YAML and semantically guards the exact branch/protection predicate, environment, runner-label set, permissions map, and artifact-download action for all three lanes. Every `uses:` step must be an owner/repository action pinned to an exact 40-hex commit; mutable refs, malformed SHAs, local actions, and Docker action refs are rejected. Negative fixtures cover those forms as well as comment smuggling, `if: false`, write permissions, misplaced/wrong actions, and missing environments.
...
| Apple Silicon macOS with Hypervisor.framework | `macos-15` feature build and entitlement inspection | Self-hosted Apple Silicon/HVF | compile and packaging lane defined; live APFS/HVF qualification has not run |
| Intel macOS | Not supported | None | runtime creation returns an explicit Apple-Silicon requirement |
...
The absence of a completed kernel-OverlayFS or Apple Silicon live run is a publication blocker, not an inferred success. Before workspace mutation, Linux opens `/dev/kvm` and requires `KVM_GET_API_VERSION == 12`; Apple Silicon queries `kern.hv_support == 1`, after which BoxLite initialization preflights/extracts its provider assets before storage creation. On macOS, BoxLite's build embeds and ad-hoc signs `boxlite-shim` with the Hypervisor entitlement; the experimental workflow validates exactly one built shim before packaging.
...
## Pinning, provenance, and publication blockers
...
BoxLite 0.9.7's build downloads `boxlite-runtime-v0.9.7-<target>.tar.gz` from its GitHub release when building from crates.io. It does not verify that archive against a Pando-controlled checksum, and the extracted runtime set contains no license/notice bundle. Consequently, `--locked` makes the Rust dependency graph repeatable but does not make the runtime build offline or supply-chain reproducible. Pando must not publish a runtime bottle until it mirrors a reviewed archive by checksum, records the complete native license/notice set, and reproduces both qualified platform builds from that mirror.

Neither Pando nor BoxLite 0.9.7 declares a Rust MSRV. The measured compiler version is evidence, not an MSRV promise; experimental builds follow the repository's stable toolchain CI until an explicit MSRV is established.
