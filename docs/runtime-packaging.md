...
override. Apple Silicon release artifacts remain host-only until the HVF lane has
passed on qualified hardware.
...
| Apple Silicon macOS with Hypervisor.framework | Experimental `macos-15` feature build and entitlement inspection | Self-hosted Apple Silicon/HVF | host-only release; live APFS/HVF qualification has not run |
...
lane rather than a claim required for rootless release support. Apple Silicon is
not advertised as runtime-capable until its live lane passes. Before workspace
