...
use crate::litebox::init::types::{resolve_user_volumes, USE_DISK_ROOTFS};
...
async fn build_config(
    ...
    // Disk-backed rootfs and user volumes provide their own guest devices. In
    // that mode `/run/boxlite/shared` is only a guest-side mountpoint, so an
    // additional empty virtiofs share wastes a scarce x86_64 virtio IRQ.
    if !USE_DISK_ROOTFS {
